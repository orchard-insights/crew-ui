<script lang="ts">
  ...
  async function retryTask () {
    ...
    try {
      ...
      await axios.post(`${apiBaseUrl}api/v1/task/${task._id}/retry`, {
...
{#if !task.isComplete && task.remainingAttempts < 1}
<Button icon={RetryFailed32} on:click={() => (open = true)} kind="tertiary" size="small" iconDescription="Retry Task" />
{/if}
